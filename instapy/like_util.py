...
def get_links_from_feed(browser, amount, num_of_search, logger):
    ...
    try:
        if link_elems:
            links = [link_elem.get_attribute('href')
...
def get_links_for_location(browser,
    ...
    if possible_posts is not None:
        possible_posts = possible_posts if not skip_top_posts else possible_posts - \
            len(top_posts)
    ...
    try:
        while filtered_links in range(1, amount):
            ...
            for i in range(3):
                ...
                update_activity()
                sc_rolled += 1
                # if not slept, and internet speed is low, instagram will only scroll one time, instead of many times you sent scroll command...
                sleep(nap)

            sleep(3)
            links.extend(get_links(browser, location,
                                   logger, media, main_elem))
            ...
            if len(links) == filtered_links:
                ...
                if try_again > 2:  # you can try again as much as you want by changing this number
                    if put_sleep < 1 and filtered_links <= 21:
                        ...
                        logger.info(
                            "'{}' location POSSIBLY has less images than desired...".format(location))
                        break
            else:
                filtered_links = len(links)
                try_again = 0
    ...
    try:
        ...
        try:
            ...
            if possible_posts:
                ...
                logger.info(
                    "Failed to get the amount of possible posts in '{}' tag  ~empty string".format(tag))
                possible_posts = None
            ...
            if len(links) == filtered_links:
                ...
                if try_again > 2:  # you can try again as much as you want by changing this number
                    if put_sleep < 1 and filtered_links <= 21:
                        ...
                        logger.info(
                            "'{}' tag POSSIBLY has less images than desired...".format(tag))
                        break
            else:
                filtered_links = len(links)
                try_again = 0
...
def get_links_for_username(browser,
    ...
    if "Page Not Found" in browser.title:
        logger.error(
            'Intagram error: The link you followed may be broken, or the page may have been removed...')
        return False

    # if private user, we can get links only if we following
    following, follow_button = get_following_status(
    ...
    while len(links) < amount:
        ...
        if len(links) == len(initial_links):
            if attempt >= 7:
                logger.info("There are possibly less posts than {} in {}'s profile page!".format(
                    amount, person))
                break
            else:
                attempt += 1
    ...
    for dont_likes_regex in dont_like_regex:
        ...
        if quash:
            ...
            iffy = ((re.split(r'\W+', dont_likes_regex))[3] if dont_likes_regex.endswith('*([^\\d\\w]|$)') else   # 'word' without format
                    # '[word'
                    (re.split(r'\W+', dont_likes_regex))[1] if dont_likes_regex.endswith('+([^\\d\\w]|$)') else
                    # ']word'
                    (re.split(r'\W+', dont_likes_regex))[3] if dont_likes_regex.startswith('#[\\d\\w]+') else
...
def verify_liking(browser, max, min, logger):
    ...
    if max is not None and likes_count > max:
        logger.info(
            "Not liked this post! ~more likes exist off maximum limit at {}".format(likes_count))
        return False
    elif min is not None and likes_count < min:
        logger.info(
            "Not liked this post! ~less likes exist off minumum limit at {}".format(likes_count))
        return False
